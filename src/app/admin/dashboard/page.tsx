...
const AdminDashboardPage = () => {
  ...
  const loadDashboardData = async () => {
    try {
      // 获取统计数据
      const [casesResult, articlesResult, downloadsResult, contactsResult] = await Promise.all([
      ...
      // 获取最近的数据
      const [recentCasesResult, recentArticlesResult, recentContactsResult] = await Promise.all([
        supabase.from('case_studies').select('*').order('created_at', { ascending: false }).limit(5),
      ...
      // 计算总下载量
      const totalDownloads = downloadsResult.data?.reduce((sum, item) => sum + (item.download_count || 0), 0) || 0;
      ...
      <div className="bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 rounded-2xl p-8 text-white">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-100 text-lg">欢迎回到CCPM360管理后台，掌控您的项目管理平台</p>
          </div>
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-bold text-gray-900 group-hover:text-gray-800 transition-colors">{card.value.toLocaleString()}</p>
                  <div className="mt-2 flex items-center text-xs text-green-600">
                    <TrendingUp className="w-3 h-3 mr-1" />
                    <span>较上月增长</span>
                  </div>
                </div>
                <div className={`w-14 h-14 bg-gradient-to-br ${gradientColors[card.color as keyof typeof gradientColors]} rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {quickActions.map((action, index) => {
            ...
            return (
              ...
              >
                <div className={`w-12 h-12 bg-gradient-to-br ${gradientColors[action.color as keyof typeof gradientColors]} rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300 shadow-lg`}>
                  <action.icon className="w-6 h-6 text-white" />
                </div>
                <h3 className="font-bold text-gray-900 mb-2 group-hover:text-gray-800 transition-colors">{action.title}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-3">
              ...
              <h2 className="text-xl font-bold text-gray-900">最近案例</h2>
            </div>
            <Link href="/admin/cases" className="text-sm font-semibold text-blue-600 hover:text-blue-500 transition-colors">
              查看全部 →
            </Link>
          </div>
          <div className="space-y-4">
            {stats.recentCases.length > 0 ? (
              stats.recentCases.map((case_study) => (
                <div key={case_study.id} className="group flex items-center space-x-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl hover:from-blue-100 hover:to-indigo-100 transition-all duration-200 border border-blue-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-600 mt-1">
                      {new Date(case_study.created_at).toLocaleDateString('zh-CN', {
                        year: 'numeric',
                        month: 'short',
                        day: 'numeric'
                      })}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/admin/resources" className="text-sm font-semibold text-green-600 hover:text-green-500 transition-colors">
              查看全部 →
            </Link>
          </div>
          <div className="space-y-4">
            {stats.recentArticles.length > 0 ? (
              stats.recentArticles.map((article) => (
                <div key={article.id} className="group flex items-center space-x-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl hover:from-green-100 hover:to-emerald-100 transition-all duration-200 border border-green-100">
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex items-center space-x-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl border border-green-100">
            ...
            <div className="flex-1">
              <span className="text-sm font-semibold text-gray-900">数据库连接</span>
          ...
          <div className="flex items-center space-x-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-100">
            ...
            <div className="flex-1">
              <span className="text-sm font-semibold text-gray-900">文件存储</span>
          ...
          <div className="flex items-center space-x-4 p-4 bg-gradient-to-r from-purple-50 to-violet-50 rounded-xl border border-purple-100">
            ...
            <div className="flex-1">
              <span className="text-sm font-semibold text-gray-900">认证服务</span>
